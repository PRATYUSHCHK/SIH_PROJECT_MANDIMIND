...
export const globalSearch = asyncHandler(async (req, res) => {
  ...
  // e.g. "₹29", "500kg", "Ravi tomato"
  const cleanQ = rawQ.replace(/[₹$,]/g, '').trim();
  const tokens = cleanQ.split(/\s+/).filter(Boolean);
  const regexTokens = tokens.map((t) => new RegExp(escapeRegex(t.replace(/kg$/i, '')), 'i'));
  ...
  // 4. LOGISTICS (from active/accepted transactions with routes)
  ...
  // 7. PAGES & NAVIGATION RECOGNITION
  const ALL_PAGES = [
    { key: 'marketplace', label: 'Marketplace', path: '/marketplace', desc: 'Direct farmer-buyer trading floor & produce listings', roles: ['seller', 'farmer', 'admin', 'buyer'], keywords: ['marketplace', 'listings', 'requirements', 'trade', 'buy', 'sell', 'produce', 'crop'] },
    { key: 'matches', label: 'AI Matches', path: '/matches', desc: 'AI-powered produce & demand compatibility engine', roles: ['seller', 'farmer', 'admin', 'buyer'], keywords: ['matches', 'matching', 'ai matches', 'deals', 'fair price', 'offers'] },
    ...
    { key: 'logistics', label: 'Logistics', path: '/logistics', desc: 'AI route calculation, cost optimization & transit', roles: ['seller', 'farmer', 'admin', 'buyer'], keywords: ['logistics', 'delivery', 'route', 'truck', 'transport', 'distance'] },
    ...
    { key: 'farmer', label: 'Farmer Mode', path: '/farmer', desc: 'Farmer crop planner, fair pricing & buyer demand', roles: ['farmer', 'admin', 'seller'], keywords: ['farmer mode', 'crop planner', 'harvest', 'farmer'] },
